...
festival_data[1] = list(dict.fromkeys(stages))
festival_data[2] = list(dict.fromkeys(restricted))
festival_data[3] = list(dict.fromkeys(attendance))
print(festival_data)
# Extension
# Write a function festival_search(item) that returns True/False if item appears in festival_data.

n = int(input("How many items are you searching for?: "))
for _ in range(n):
